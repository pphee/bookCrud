...
ifeq ($(OS), Windows_NT)
    ...
    OS = $(shell (Get-ComputerInfo -Property OsVersion, OsArchitecture | Format-Table -HideTableHeaders | Out-String).Trim())
...
.DEFAULT_GOAL := help
.PHONY: greet schoolcrud calculator help
project := greet calculator schoolcrud

all: $(project) ## Generate Pbs and build

greet: $@ ## Generate Pbs and build for greet
calculator: $@ ## Generate Pbs and build for calculator
schoolcrud: $@ ## Generate Pbs and build for schoolcrud
...
clean: clean_greet clean_calculator clean_schoolcrud ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
